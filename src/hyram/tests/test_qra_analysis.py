...
import hyram.qra.analysis as hyram_analysis
import hyram.qra.component_failure as hyram_component_failure
import hyram.qra.component_set as hyram_component_set
import hyram.qra.data.component_data as hyram_component_data
...
class TestAnalysis(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.component_failure_set = hyram_component_failure.ComponentFailureSet(
            ...
            coupling_ftc_a=0.5,
            coupling_ftc_b=5031)
        self.leak_sizes = None  # Use default: [0.01, 0.10, 1.00, 10.00, 100.00]
        self.rel_species = 'h2'
        ...
        number_of_extra2s = 0
        self.component_sets = [
            hyram_component_set.ComponentSet(category='compressor',
                                             num_components=number_of_compressors,
                                             species=self.rel_species,
                                             saturated_phase=self.rel_phase,
                                             leak_frequency_lists=hyram_component_data.h2_gas_params['compressor'],
                                             leak_sizes=self.leak_sizes),
            hyram_component_set.ComponentSet(category='vessel',
                                             num_components=number_of_vessels,
                                             species=self.rel_species,
                                             saturated_phase=self.rel_phase,
                                             leak_frequency_lists=hyram_component_data.h2_gas_params['vessel'],
                                             leak_sizes=self.leak_sizes),
            hyram_component_set.ComponentSet(category='valve',
                                             num_components=number_of_valves,
                                             species=self.rel_species,
                                             saturated_phase=self.rel_phase,
                                             leak_frequency_lists=hyram_component_data.h2_gas_params['valve'],
                                             leak_sizes=self.leak_sizes),
            hyram_component_set.ComponentSet(category='instrument',
                                             num_components=number_of_instruments,
                                             species=self.rel_species,
                                             saturated_phase=self.rel_phase,
                                             leak_frequency_lists=hyram_component_data.h2_gas_params['instrument'],
                                             leak_sizes=self.leak_sizes),
            hyram_component_set.ComponentSet(category='joint',
                                             num_components=number_of_joints,
                                             species=self.rel_species,
                                             saturated_phase=self.rel_phase,
                                             leak_frequency_lists=hyram_component_data.h2_gas_params['joint'],
                                             leak_sizes=self.leak_sizes),
            hyram_component_set.ComponentSet(category='hose',
                                             num_components=number_of_hoses,
                                             species=self.rel_species,
                                             saturated_phase=self.rel_phase,
                                             leak_frequency_lists=hyram_component_data.h2_gas_params['hose'],
                                             leak_sizes=self.leak_sizes),
            hyram_component_set.ComponentSet(category='pipe',
                                             num_components=number_of_pipes,
                                             species=self.rel_species,
                                             saturated_phase=self.rel_phase,
                                             leak_frequency_lists=hyram_component_data.h2_gas_params['pipe'],
                                             leak_sizes=self.leak_sizes),
            hyram_component_set.ComponentSet(category='filter',
                                             num_components=number_of_filters,
                                             species=self.rel_species,
                                             saturated_phase=self.rel_phase,
                                             leak_frequency_lists=hyram_component_data.h2_gas_params['filter'],
                                             leak_sizes=self.leak_sizes),
            hyram_component_set.ComponentSet(category='flange',
                                             num_components=number_of_flanges,
                                             species=self.rel_species,
                                             saturated_phase=self.rel_phase,
                                             leak_frequency_lists=hyram_component_data.h2_gas_params['flange'],
                                             leak_sizes=self.leak_sizes),
            hyram_component_set.ComponentSet(category='exchanger',
                                             num_components=number_of_heat_exchangers,
                                             species=self.rel_species,
                                             saturated_phase=self.rel_phase,
                                             leak_frequency_lists=hyram_component_data.h2_gas_params['exchanger'],
                                             leak_sizes=self.leak_sizes),
            hyram_component_set.ComponentSet(category='vaporizer',
                                             num_components=number_of_vaporizers,
                                             species=self.rel_species,
                                             saturated_phase=self.rel_phase,
                                             leak_frequency_lists=hyram_component_data.h2_gas_params['vaporizer'],
                                             leak_sizes=self.leak_sizes),
            hyram_component_set.ComponentSet(category='arm',
                                             num_components=number_of_transfer_arms,
                                             species=self.rel_species,
                                             saturated_phase=self.rel_phase,
                                             leak_frequency_lists=hyram_component_data.h2_gas_params['arm'],
                                             leak_sizes=self.leak_sizes),
            hyram_component_set.ComponentSet(category='extra1',
                                             num_components=number_of_extra1s,
                                             species=self.rel_species,
                                             saturated_phase=self.rel_phase,
                                             leak_frequency_lists=hyram_component_data.h2_gas_params['extra1'],
                                             leak_sizes=self.leak_sizes),
            hyram_component_set.ComponentSet(category='extra2',
                                             num_components=number_of_extra2s,
                                             species=self.rel_species,
                                             saturated_phase=self.rel_phase,
                                             leak_frequency_lists=hyram_component_data.h2_gas_params['extra2'],
                                             leak_sizes=self.leak_sizes)
        ]
    ...
    def test_zero_risk_for_zero_components_zero_refuelings(self):
        ...
        zero_refuelings_component_failure_set = hyram_component_failure.ComponentFailureSet(num_vehicles=0)
        results = hyram_analysis.conduct_analysis(self.pipe_outer_diam,
                                                  self.pipe_thickness,
                                                  self.amb_temp,
                                                  self.amb_pres,
                                                  self.rel_temp,
        ...
        self.assertAlmostEqual(risk_value, 0.0)
    ...
    def test_reject_bauwens_with_impulse_probits(self):
        ...
        for probit_overp_id in probit_overp_ids:
            self.assertRaises(ValueError,
                              hyram_analysis.conduct_analysis,
