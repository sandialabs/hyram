...
def dP_expansion(enclosure, mass, gas):
    ...
    prod = products_hydrogen_stoich(gas.T, gas.species)
    ...
    deltaP  = gas.P*((((Vol_total+Vol_gas)/Vol_total)*((Vol_total+VolStoich*(sigma-1))/Vol_total))**gamma-1)
    return deltaP
...
#   species:Specifies the fuel (hydrogen, methane)
#
# Outputs:
...
def products_hydrogen_stoich(T0,species):
    MW_O2   = 31.999
    MW_N2   = 28.013
    MW_CO2 = 44.01
    MW_H2O = 18.015
    Xi_O2_a = 0.2095
    if species == 'hydrogen' or species == 'H2':
        MW_f= 2.016
        DHc = 119.0e6  # heat of combustion [J/kg]
        cnum= 0.
        hnum= 2.
    elif species == 'methane' or species == 'CH4':
        MW_f= 16.04
    ...
    DHc     = DHc*Y_u[2]
    Cp_u    = computeCp(Xi_u,T0,MW,species)
    # [J/(kg K)]
    R_u     = Ru/MW_u
    ...
    Cpguess = computeCp(Xi_b,T0,MW,species)  # [J/(kg K)]
    Tguess  = H_b/Cpguess 
    f0      = lambda T: computeT(T,H_b,Xi_b,MW,species)
...
def computeT(T,H_b,Xi_b,MW,species):
    Cp  = computeCp(Xi_b,T,MW,species)
    y   = Cp*T - H_b
    return y

# Compute specific heat for each species (data from CHEMKIN thermo file)
def computeCp(Xi,T,MW,species): # [J/(kg K)]
    O2  = 0
    N2  = 1
    CH4 = 2
    H2O = 3
