...
def conduct_analysis(pipe_outer_diam, pipe_thickness,
    ...
    for group_dict in occupant_input_list:
        ...
        occupant_hours += int(group_dict['hours'] * num_occupants)
    occupant_avg_hours = occupant_hours / total_occupants
    ...
    for leak_diam in orifice_leak_diams:
        discharge_rate = _comps.Orifice(leak_diam, discharge_coeff).compute_steady_state_mass_flow(rel_fluid)
    ...
    flame_data = FlameData(amb_temp, amb_pres,
                           rel_species, rel_temp, rel_pres, rel_phase,
                           orifice_leak_diams, leak_height,
                           rel_angle,
    ...
    try:
        ...
        log.info("Flame load error. Re-computing...")
        prev_flame_saved = False
    ...
    if prev_flame_saved:
        ...
        positions = loaded_flame_data.positions
        ...
        positions = flux_dict['positions'].T
        # Pickle it for later
        flame_data.qrads = qrads
        flame_data.plot_files = qrad_plot_files
        flame_data.positions = positions
    ...
    position_qrads_kw = (qrads.reshape((num_leak_sizes, total_occupants)) / 1000.).T
    ...
    results = {
        ...
        'positions': positions,
        'position_qrads': position_qrads_kw
    }
    ...
    for i, leak_size in enumerate(leak_sizes):
        ...
        leak_result.leak_diam = orifice_leak_diams[i]
