...
class DevelopingFlow:
    def __init__(self, fluid, orifice, ambient, mdot=None,
        ...
        self.orifice_flow = NozzleFlow(fluid, self.orifice, ambient.P, mdot, suppressWarnings) # plug node at orifice exit
        self.fluid_orifice = self.orifice_flow.fluid
        
        self.mass_flow_rate = self.orifice_flow.mdot
        if self.verbose:
            print('done')
        self.d0 = orifice.d
        ...
        # Underexpanded jet (gets fluid to atmospheric pressure (if needed, otherwise retains same properties))
        self.fluid_exp, self.orifice_exp = self._expand(ambient, nn_T, nn_conserve_momentum)
    ...
    def theta0(self, val):
        ...
        SE = self.initial_node.S - self.expanded_plug_node.S
    ...
    #todo: could probably improve orifice.setter - can likely redo _expand with less overhead - if notional nozzle models were modified
    @orifice.setter
    def orifice(self, new_orifice):
        self.orifice_flow.orifice = new_orifice
        self.mass_flow_rate = self.orifice_flow.mdot
        self.fluid_orifice = self.orifice_flow.fluid
        ...
        self.orifice_node = PlugNode(self.d0*np.sqrt(new_orifice.Cd), self.fluid_orifice.v, self.fluid_orifice.rho,# diameter scaled to account for Cd
                                     1, self.fluid_orifice.T, self.theta0, self.x0, self.y0, 0)
        # Underexpanded jet (if needed: gets fluid to atmospheric pressure)
        self.fluid_exp, self.orifice_exp = self._expand(self.ambient, self._nn_T, self._nn_conserve_momentum)
    ...
    def _expand(self, ambient, nn_T, nn_conserve_momentum):
        '''
        ...
        if self.fluid_orifice.P > ambient.P: # use notional nozzle model
            ...
            nn = NotionalNozzle(self.orifice_flow, ambient)
            g, o = nn.calculate(nn_T, nn_conserve_momentum)
            if self.verbose:
                print('done.')
        else:
            g, o = self.fluid_orifice, self.orifice
        return g, o
...
class PlugNode:
    ...
    def establish(self, ambient, fluid, lam):
        ...
        Fr_den = self.Froude(ambient.rho)
        SE = self.d*((6.2                  ) if (Fr_den**2 >= 40) else
                     (3.9 + 0.057*Fr_den**2) if (5<=Fr_den**2<40) else
                     (2.075+0.425*Fr_den**2) if (1<=Fr_den**2<5 ) else
                     (0                    ))
...
class GaussianNode:
    ...
    def entrainment(self, Emom, rho_amb, alpha_buoy, alpha):
        Fr_L = self.v_cl**2*self.rho_cl/(const.g*self.B*abs(rho_amb-self.rho_cl)) # ESH: added absolute value 07/23/20 - for negatively buoyant jets, this was a negative number - might need justification
        E_buoy = alpha_buoy/Fr_L*(2*const.pi*self.v_cl*self.B)*np.sin(self.theta)  # m**2/s
        E = Emom + E_buoy
...
class Jet:
    def __init__(self, fluid, orifice, ambient, mdot=None,
        ...
        Fr_den = self.developing_flow.expanded_plug_node.Froude(ambient.rho)
        self.Fr_den = Fr_den
        if Fr_den < 268:
            self._alpha_buoy = 17.313-0.11665*Fr_den+(2.0771e-4)*Fr_den**2
        else:
    ...
    def m_flammable(self, X_lean=None, X_rich=None, Hmax=np.inf):
        ...
        if np.all(self.y > Hmax) or np.all(self.Y_cl < Ylean): # no flammable mass
            return 0

        # Trim the plume down to below Hmax:
        def trim_below(Hmax, y_vals, cl_vals):
            return np.append(cl_vals[np.argwhere(y_vals < Hmax)].T[0], np.interp(Hmax, y_vals, cl_vals))
        if np.any(self.y > Hmax):
            S = trim_below(Hmax, self.y, S)
            Y_cl = trim_below(Hmax, self.y, Y_cl)
            B = trim_below(Hmax, self.y, B)
            rho_cl = trim_below(Hmax, self.y, rho_cl)

        def rho(r, i):
            '''density at radius: r, for plume at node: i'''
            return (rho_cl[i] - self.ambient.rho) * np.exp(-r**2 / (self.lam * B[i]**2)) + self.ambient.rho
        def Y(r, i):
            '''mass fraction at radius: r, for plume at node: i'''
            return rho_cl[i] * Y_cl[i] * np.exp(-r**2 / (self.lam * B[i])**2) / rho(r, i)

        Slean = np.interp(Ylean, Y_cl[::-1], S[::-1])
        ivals = slice(0, np.argmax(Y_cl <= Ylean))
        def trim_cl_lean(var):
            return np.append(var[ivals], np.interp(Slean, S, var))
        Y_cl = trim_cl_lean(Y_cl)
        B = trim_cl_lean(B)
        rho_cl = trim_cl_lean(rho_cl)
        S = trim_cl_lean(S)

        def rval_at_Yval(Yval):
            if Y(0, i) <= Yval:
                rval = 0
            else:
                rval = optimize.brentq(lambda r: Y(r, i) - Yval, 0, 100*B[i])
            return rval
        def mass_per_cylindrical_shell(r):
            return Y(r, i) * rho(r, i) * 2 * const.pi * r
        mass_per_len = []
        # integrate to find the mass/length at each node
        for i in range(len(S)):
            r_lean_i = rval_at_Yval(Ylean)
            r_rich_i = rval_at_Yval(Yrich)
            mass_per_len_i = integrate.quad(mass_per_cylindrical_shell, r_rich_i, r_lean_i)[0]
            mass_per_len.append(mass_per_len_i)
    ...
    def get_xy_distances_to_mole_fractions(self, fracs):
        ...
        """
        contours = [fracs] if type(fracs) in [int, float] else fracs
        x_values, y_values, molefrac_values, __, __, __ = self._contourdata
        cg = contour_generator(x_values, y_values, molefrac_values)
        distances = {}

        for contour in contours:
            try:
                xs, ys = cg.lines(contour)[0].T
            except:
                raise ValueError(f"There doesn't seem to be a contour at {contour}, try a value between {molefrac_values.min()} and {molefrac_values.max()}.")
