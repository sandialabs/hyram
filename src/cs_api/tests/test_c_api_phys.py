...
class IndoorReleaseTestCase(unittest.TestCase):
    ...
    def test_default(self):
        ...
        wrapped = phys.analyze_accumulation(amb_temp, amb_pres,
...
class TestFlameTempPlotGeneration(unittest.TestCase):
    ...
    def test_default(self):
        wrapped = phys.jet_flame_analysis(**self.params)
...
class TestRadHeatAnalysis(unittest.TestCase):
    ...
    def test_default(self):
        ...
        radiant_frac = data['radiant_frac']
        ...
        self.assertTrue(radiant_frac > 0)
        self.assertFalse(warning)

    def test_sat_liquid(self):
        self.params |= {'rel_pres': 5*const.atm, 'rel_temp': None, 'rel_phase': 'liquid'}
        wrapped = phys.jet_flame_analysis(**self.params)
    ...
    def test_sat_vapor_birch(self):
        self.params |= {'rel_pres': 1e6, 'rel_temp': None, 'rel_phase': 'gas', 'nozzle_key': 'bir'}
        wrapped = phys.jet_flame_analysis(**self.params)
        data = wrapped["data"]
        flux = data["flux_data"]
        flux_filepath = data["flux_plot_filepath"]
