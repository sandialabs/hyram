                ...
                if (e.ColumnIndex == dataColumnindex || e.ColumnIndex == unitColumnindex)
                {
                    ...
                    if (activeRow.Tag != null)
                    {
                        ...
                        if (activeRow.Cells[e.ColumnIndex] is DataGridViewComboBoxCell)
                        {
                            ...
                            var newValue = QraStateContainer.Instance.GetStateDefinedValueObject(vdo.Key.ToUpper())
                                .GetValue(unitType)[0];

                            UiStateRoutines.FillUserParamsFromString(newValue.ToString(), vdo.Key, converter, unitType);

                            vdo.Unit = unitType;
                            ((Dictionary<string, Enum>) QraStateContainer.Instance.Parameters["VarUnitDict"])[vdo.Key] =
                                unitType;
                        }
                        ...
                        activeRow.Cells[dataColumnindex].Value = QraStateContainer.Instance
