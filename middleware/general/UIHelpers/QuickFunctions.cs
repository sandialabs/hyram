...
namespace UIHelpers
{
    ...
    public class QuickFunctions
    {
        ...
        private static double GetNumericPartOfCellValueString(string cellValue)
        {
            ...
            if (!Parsing.TryParseDouble(cellValue, out result))
                if (cellValue.Length > 0)
                {
                    var numericPart = GetPrependedNumericPartOfString(cellValue);
                    if (numericPart.Length > 0) Parsing.TryParseDouble(numericPart, out result);
                }
        ...
        private static string GetPrependedNumericPartOfString(string cellValue)
        {
            ...
            for (var index = 0; index < cellValue.Length; index++)
            {
                ...
                if (MiscFunctions.IsParseableNumber(result + thisCh))
                    result += thisCh;
                else
                    break;
            }
        ...
        public static void ClearOldTemporaryFiles()
        {
            // Get top temp folder
            string folderNameToClear = StringFunctions.RemoveTrailingBackslash(Application.LocalUserAppDataPath) + "\\Temp";;
            if (Directory.Exists(folderNameToClear))
                foreach (var thisFolder in Directory.GetDirectories(folderNameToClear))
                {
                    var twoDaysAgo = DateTime.Now.Subtract(new TimeSpan(24 * 2, 0, 0));
                    var folderTime = Directory.GetLastAccessTime(thisFolder);
                    if (folderTime < twoDaysAgo) Directory.Delete(thisFolder, true);
                }
        }
    }
