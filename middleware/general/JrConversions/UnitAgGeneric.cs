...
namespace JrConversions
{
    ...
    [Serializable]
    public class UnitOfMeasurementConverters : Dictionary<string, NdDualConversionProvider>, ISerializable
    {
        ...
        public UnitOfMeasurementConverters(SerializationInfo info, StreamingContext context)
        {
            ...
            for (var index = 0; index < theKeys.Length; index++)
            {
                ...
                Add(theKeys[index], (NdDualConversionProvider) oThisNode);
            }
        }
        ...
        public double[] ConvertFromSi(string destinationUnitName, double[] value)
        {
            ...
            double[] result = null;
            if (conversionProvider.ConversionDataObject != null)
            {
                result = new double[value.Length];
                for (var index = 0; index < value.Length; index++)
                    result[index] = value[index] / conversionProvider.ConversionDataObject.ConversionFactor;
            }
            else
            {
        ...
        public double[] ConvertToSi(string sourceUnitName, double[] value)
        {
            ...
            for (var index = 0; index < value.Length; index++)
                result[index] = value[index] * converter.ConversionDataObject.ConversionFactor;

            return result;
        }

        private NdDualConversionProvider GetConverter(string typeName)
        {
    ...
    [Serializable]
    public class NdUnitAgConversionData
    {
