...
class Leak(object):
    ...
    def __init__(self, size, mu, sigma, description='', mean=None, variance=None):
        self.description = str(description)

        valid, error_msg = self.parameters_valid(size, mu, sigma, mean, variance)
        if not valid:
            log.error(error_msg)
    ...
    def parameters_valid(self, size, mu, sigma, mean, variance):
        ...
        elif (mu is None or sigma is None) and (mean is None or variance is None):
            valid = False
            msg = ("Leak probability parameters for size {}% invalid. Must include mu and sigma or mean and variance."
                   "Passed values for mu/sigma/mean/variance are {}, {}, {}, {}"
                   ).format(size, mu, sigma, mean, variance)
...
class LeakSet(object):
    ...
    def __init__(self, parent, component_leak_probs, leak_sizes=None):
        ...
        for i, leak_size in enumerate(leak_sizes):
            comp_prob = component_leak_probs[i]
            # mu or sigma may be none or empty; if so, use mean and variance
            mu = comp_prob['mu']
            sigma = comp_prob['sigma']
            mean = None
            variance = None

            # TODO: move some of this to validation
            if mu == 0. and sigma == 0.:
                mu = None
                sigma = None
                mean = 0.
            ...
            elif mu is None or mu == "" or mu == np.nan or sigma is None or sigma == "" or sigma == np.nan:
                # mu/sigma not given so get mean/variance directly
                mu = None
                sigma = None
                if 'mean' in comp_prob:
                    mean = comp_prob['mean']
                else:
                    msg = "Component {} probability set for leak {} is invalid (mean): {}".format(self.parent.category,
                                                                                                  leak_size, comp_prob)
                    log.error(msg)
                    raise KeyError(msg)
                if 'variance' in comp_prob:
                    variance = comp_prob['variance']
                else:
                    msg = "Component {} probability set for leak {} is invalid (var): {}".format(self.parent.category,
                                                                                                  leak_size, comp_prob)
                    log.error(msg)
                    raise KeyError(msg)

            leak = Leak(leak_size, mu, sigma, mean=mean, variance=variance)
