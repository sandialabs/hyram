...
# TODO remove vtk dependency?
#import vtk
#vtk.vtkObject.GlobalWarningDisplayOff()

class Flame:
    def __init__(self, gas, orifice, ambient, Q = None, 
                 theta0 = 0., x0 = 0, y0 = 0, nnmodel = 'YuceilOtugen',
                 h2chem = None, 
                 lamf = 1.24, lamv = 1.24, 
                 am = 3.42e-2, ab = 0.000575, af = 0.23,
                 numpts = 100, numB = 6, numBpts = 10000,
                 betaS = 8.6, betaB = 0.8,
                 verbose = True):
        '''
        class for calculating the characteristics of a 2-D flame, 
            without wind 
        see Ekoto et al. International Journal of Hydrogen Energy, 
            39, 2014 (20570-20577)
        
        Parameters
        ----------
        gas: Gas object (h2_comps)
            gas that is being released
        orifice: Orifice object (h2_comps)
            orifice through which gas is being released
        ambient: Gas object (h2_comps)
            gas into which release is occuring
        Q: float, optional if gas.P >= CRITICAL_RATIO_H2*ambient.P
            volumetric flow rate of subsonic release (m^3/s)
        ...
        h2chem : hydrogen chemistry class (see h2_therm for usage)
            optional, if none given, will initialize new chemisty class
        lam : float
            spreading ratio for mixture fraction 
            and velocity Gaussian profiles
        am : float
        ...
        nnmodel : string
            notional nozzle model to use for high-pressure release 
            (see h2_nn for usage)
        ...
        betaS : float, optional
            proportionality constant for start of integral model 
            relative to d* (see Li, et al. IJHE 2015)
        betaB : float, optional
            proportionality constant for width at start of integral 
            model relative to d* (see Li, et al. IJHE 2015)
        '''
        ...
        if gas.P < CriticalRatio*ambient.P and Q is None:
            raise ValueError('underspecified, unchoked, please specify flow rate')
        if gas.P >= CriticalRatio*ambient.P and Q is None:
            # Notional nozzle:
        ...
        self.amb = ambient
        # TODO why V^2????
        self.Emom = am*np.sqrt(self.orifice1.A*(self.gas1.rho/self.amb.rho)
                               *self.V1**2)
        ...
        self.V_cl, self.B, self.theta, self.f_cl = [], [], [], []
        self.x, self.y, self.S = [], [], []
        self.verbose = verbose
        
    def _govEqns(self, S, ind_vars, numB = 6, numpts = 500):
        '''
        Governing equations for a flame, 
        written in terms of d/dS of (V_cl, B, theta, f_cl, x, and y).
        
        A matrix soluition to the continuity, x-momentum, y-mometum, 
        and mixture fraction equations
        Solves for d/dS of the dependent variables V_cl, B, theta, 
        and f_cl.  
        Numerically integrated to infinity = numB * B(S) using numpts 
        discrete points.
        '''
        ...
        r = np.zeros(numpts)
        r[1:] = np.logspace(-5, np.log10(numB*B), numpts-1)
        ...
        rho = self.h2chem.rho_prod(f)
        drhodf = self.h2chem.drhodf(f)
        
        rho_temp = self.amb.rho - rho
        rho_int = integrate.trapz(rho_temp/B, r)
        
        Ebuoy = 2*const.pi*self.ab*np.sin(theta)*(const.g*(rho_int)/(V_cl*self.gas1.rho))# m**2/s
        E = self.Emom + Ebuoy
    ...
    def solve(self, Smax = None, tol = 1e-5, establish = 'old'):
        ...
        '''
        try:
            if self.h2chem.Treac != self.amb.T or abs(self.h2chem.P/self.amb.P - 1) > 1e-10:
                self.h2chem.reinitilize(self.amb.T, self.amb.P)
        except:
            self.h2chem = H2_Combustion(self.amb.T, self.amb.P)
        if self.verbose:
        ...
        if establish == 'old':
            ...
            f_clE = (self.lamf**2+1)/(2*self.lamf**2)
            ...
            V_clE = (self.lamv**2 + 1)/self.lamv**2*self.gas1.rho*self.orifice1.d**2*self.V1/(4*rho_clE*f_clE*BE**2)
        thetaE = self.theta0
    ...
    def length(self):
        ...
        try:
            ...
            self.h2chem = H2_Combustion(self.amb.T, self.amb.P)
        fs, Tad = self.h2chem.fstoich, self.h2chem.T_prod(self.h2chem.fstoich)
        Tamb = self.amb.T
        rhoair, rhof = self.amb.rho, self.h2chem.rho_prod(self.h2chem.fstoich)
        Deff, rhoeff = self.orifice1.d, self.gas1.rho
        
        # Compute the flame Froude number
        DTf = Tad - Tamb # change between ambient and adiabatic flame temperatures
        Frf = (self.V1*fs**1.5)/(((rhoeff/rhoair)**0.25)*np.sqrt((DTf/Tamb)*const.g*Deff))
        ...
        self.tauf = (const.pi/12)*(rhof*(self.Wf**2)*self.Lvis*fs)/(self.orifice1.mdot(self.gas1.rho, self.V1))*1000
    ...
    def Qrad_multi(self, x, y, z, RH, WaistLoc = 0.75, N = 50):
        ...
        DHc = self.h2chem.DHc  # heat of combustion [J/kg]
        try:
            Lvis = self.Lvis  # length of visible flame [m]
        except:
            Lvis = self.length()
        Xrad = self.Xrad
        mdot = self.orifice1.mdot(self.gas1.rho, self.V1)  # mass flow rate [kg/s]
        T = self.amb.T
        ...
        try:
            S = np.linspace(self.S[0],self.S[0]+Lvis,N)
    ...
    def _contourdata(self):
        ...
        r = np.append(np.append(-np.logspace(np.log10(10*max(self.B)), -5), 0), np.logspace(-5, np.log10(10*max(self.B))))
        ...
        Tvals = self.h2chem.T_prod(fvals)
    ...
    def iso_heat_flux_plot_sliced(self, title,
                                  flux3d_filepath, flux2d_filepath,
                                  smodel = 'multi',
                                  RH = 0.89,
                                  contours=None,
                                  nx = 50, ny = 50, nz = 50):
        ...
        title: string (optional)
            title shown on plot
        flux3d_filepath: string, optional
            filepath of 3D flux plot
        flux2d_filepath: string, optional
        ...
        plt.savefig(flux3d_filepath, dpi = 200)
        ...
        plt.savefig(flux2d_filepath, dpi=200)
